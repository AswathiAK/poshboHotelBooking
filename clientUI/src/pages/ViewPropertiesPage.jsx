          ...
          <div className="flex items-center justify-center h-full">
            ...
            <div className='my-8'>
                {list.length > 0 && list.map(item => (
                  <div key={item._id} className="flex gap-5 relative">                    
                    <Link to={`/host/view_property/${item._id}`}                     
                      className="flex items-center border rounded-xl p-4 mb-4 gap-8"
                    >
                      <div className="flex items-center justify-center w-40 h-40 border rounded-md shrink-0">
                        <img src={item.photos[0]} alt="" className='p-1 w-36 h-36 object-cover'/>
                      </div>
